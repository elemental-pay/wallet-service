/* Code from https://github.com/zingolabs/zingo-pc/blob/0b64b58e7720ae571415daddc2a71c1d92b630bc/src/rpc/rpc.ts
  zingo-pc license - MIT License
*/
import {
  TotalBalance,
  Transaction,
...
import { zingolib_execute_spawn, zingolib_execute_async, zingolib_deinitialize } from '@elemental-zcash/zingo-node';
// const  = pkg;
...
export default class RPC {
  ...
  static doSync() {
    const syncstr: string = zingolib_execute_spawn("sync", "");
    console.log(`Sync exec result: ${syncstr}`);
  }

  static doRescan() {
    const syncstr: string = zingolib_execute_spawn("rescan", "");
    console.log(`rescan exec result: ${syncstr}`);
  }

  static async doSyncStatus(): Promise<string> {
    const syncstr: string = await zingolib_execute_async("syncstatus", "");
    console.log(`sync status: ${syncstr}`);
    return syncstr;
  }

  static async doSave() {
    const savestr: string = await zingolib_execute_async("save", "");
    console.log(`Save status: ${savestr}`);
  }

  static deinitialize() {
    const str: string = zingolib_deinitialize();
  ...
  async shieldAllBalanceToOrchard(): Promise<string> {
    try {
      const result: string = await zingolib_execute_async("shield", 'all');
      this.updateData();
      return result;
    } catch(error: any) {
  ...
  async shieldTransparentBalanceToOrchard(): Promise<string> {
    try {
      const result: string = await zingolib_execute_async("shield", 'transparent');
      this.updateData();
      return result;
    } catch(error: any) {
  ...
  async shieldSaplingBalanceToOrchard(): Promise<string> {
    try {
      const result: string = await zingolib_execute_async("shield", 'sapling');
      this.updateData();
      return result;
    } catch(error: any) {
  ...
  static async getInfoObject(): Promise<Info> {
    const infostr: string = await zingolib_execute_async("info", "");
    ...
    try {
      ...
      const resultStr: string = await zingolib_execute_async("updatecurrentprice", "");
      if (resultStr) {
      ...
      //default fee
      const feeStr: string = await zingolib_execute_async("defaultfee", "");
      ...
      //zingolib version
      let zingolibStr: string = await zingolib_execute_async("version", "");
      if (zingolibStr) {
  ...
  static doImportPrivKey(key: string, birthday: string): string {
    ...
    const address: string = zingolib_execute_spawn("import", JSON.stringify(args));

    return address;
  }

  async fetchWalletSettings() {
    const download_memos_str: string = await zingolib_execute_async("getoption", "download_memos");
    const download_memos = JSON.parse(download_memos_str).download_memos;

    let transaction_filter_threshold = 0;
    try {
      const spam_filter_str: string = await zingolib_execute_async("getoption", "transaction_filter_threshold");
  ...
  static async setWalletSettingOption(name: string, value: string): Promise<string> {
    const r: string = await zingolib_execute_async("setoption", `${name}=${value}`);
  ...
  async zingolibBalance(): Promise<any> {
    const balanceStr: string = await zingolib_execute_async("balance", "");
    ...
    // fetch all addresses
    const addressesStr: string = await zingolib_execute_async("addresses", "");
    const addressesJSON = JSON.parse(addressesStr);

    // fetch all notes
    const notesStr: string = await zingolib_execute_async("notes", "");
  ...
  async zingolibTxList(): Promise<any> {
    ...
    const txListStr: string = await zingolib_execute_async("list", "");
  ...
  async fetchTotalBalance() {
    //const balanceStr = zingolib_execute_async("balance", "");
    //const balanceJSON = JSON.parse(balanceStr);
    ...
    // const pendingNotes = zingolib_execute_async("notes", "");
  ...
  static async getPrivKeyAsString(address: string): Promise<string> {
    const privKeyStr: string = await zingolib_execute_async("export", address);
  ...
  static async getUfvk(): Promise<string> {
    const privKeyStr: string = await zingolib_execute_async("exportufvk", '');
    const privKeyJSON = JSON.parse(privKeyStr);

    return privKeyJSON.ufvk;
  }
  ...
  static async createNewAddress(type: AddressType) {
    ...
    const addrStr: string = await zingolib_execute_async(
      "new",
      type === AddressType.unified ? "ozt" : type === AddressType.sapling ? "zt" : "ot"
    );
  ...
  static async encryptMessage(address: Address, memo: string) {
    const encryptMessageJson = { address, memo };

    console.log({ encryptMessageJson })
    const messageStr: string = await zingolib_execute_async(
      "encryptmessage",
      JSON.stringify(encryptMessageJson),
    );
  ...
  static async fetchSeed(): Promise<string> {
    const seedStr: string = await zingolib_execute_async("seed", "");
  ...
  static async fetchWalletHeight(): Promise<number> {
    const heightStr: string = await zingolib_execute_async("height", "");
  ...
  async fetchTandZTransactions(latestBlockHeight: number) {
    // const listStr = zingolib_execute_async("list", "");
  ...
  async sendTransaction(sendJson: SendManyJsonType[], setSendProgress: (p?: SendProgress) => void): Promise<string> {
    ...
    const prevProgressStr: string = await zingolib_execute_async("sendprogress", "");
    ...
    try {
      ...
      const resp: string = await zingolib_execute_async("send", JSON.stringify(sendJson));
    ...
    const sendTxPromise: Promise<string> = new Promise((resolve, reject) => {
      const intervalID = setInterval(async () => {
        const progressStr: string = await zingolib_execute_async("sendprogress", "");
  ...
  async encryptWallet(password: string): Promise<boolean> {
    const resultStr: string = await zingolib_execute_async("encrypt", password);
  ...
  async decryptWallet(password: string): Promise<boolean> {
    const resultStr: string = await zingolib_execute_async("decrypt", password);
    const resultJSON = JSON.parse(resultStr);
  ...
  async lockWallet(): Promise<boolean> {
    const resultStr: string = await zingolib_execute_async("lock", "");
    const resultJSON = JSON.parse(resultStr);
  ...
  async unlockWallet(password: string): Promise<boolean> {
    const resultStr: string = await zingolib_execute_async("unlock", password);
    const resultJSON = JSON.parse(resultStr);
